...
from functionUtils import DELAYED as D
...
class Grader():
    ...
    def grade_assignment(self, to_grade_func, params, assignment_name, err_funcs, expected_results, repeats=1):        
        ...
        for p, result, func_error in zip(params, expected_results, err_funcs):
            time_to_run = repeats * 10
            if 'maxtime' in p:
                time_to_run = (repeats+1)*p['maxtime'] + 0.02 
            execnum += 1
            report = {
            ...
            try:
                ...
                @exit_after(time_to_run)
                def run_and_check():
                    for i in range(repeats):
                        gc.collect()
                        _res = run_func(to_grade_func, p)
                    _error = func_error(_res, result)
                    return _res, _error

                res, error = run_and_check()
                end = time.time()
                ...
                error = "running took more then " + str(time_to_run) + " seconds"
    ...
    def grade_assignment_1(self):
        try:
            ...
            names = ('f', 'a', 'b', 'n')
            valss = [(R(10 * repeats)(f2), 0, 5, 10),
                     (R(20 * repeats)(f4), -2, 4, 20),
                     (R(50 * repeats)(f3), -1, 5, 50),
                     (R(20 * repeats)(f13), 3, 10, 20),
                     (R(20 * repeats)(f1), 2, 5, 20),
                     (R(10 * repeats)(f7), 3, 16, 10),
                     (R(10 * repeats)(f8), 1, 2, 10),
                     (R(10 * repeats)(f9), 5, 10, 10),
                     ]
    ...
    def grade_assignment_3_integrate(self):
        try:
            ...
            self.add_error_report('Assignment 3 integrate', 'integrate', e, 1)
    ...
    def grade_assignment_4(self):
        try:
            ...
            delay = 0.005
            valss = [(D(delay)(f2_noise), 0, 5, 2, 5),
                     (D(delay)(f13_noise), 3, 10, 2, 10),  ##
                     (D(delay)(f12_noise), 1, 8, 2, 10),  ##
                     (D(delay)(f1_noise), -12, 25, 0, 5), # 
                     (D(delay)(f7_noise), 3, 16, 10, 7),  ###
                     ]
            params = [dict(zip(names, vals)) for vals in valss]
            expected_results = [f2, f13, f12, f1, f7]

            def err(fres, fexp, f, a, b, n, t): 
                return sum([
                    abs(fres(x) - fexp(x))
                    for x in uniform(low=a, high=b, size=2 * n)
                ]) / 2 / n if n!=0 else \
            ...
            func_error = [  # mean absolute error at 2n points within the [a,b] range
                SAVEARGS(f=f, a=a, b=b, n=n, t=t)(err)
    ...
    def grade(self):
        print("grade_assignment_1")
        self.grade_assignment_1()
        print("grade_assignment_2")
        self.grade_assignment_2()
        print("grade_assignment_3_integrate")
        self.grade_assignment_3_integrate()
        print("grade_assignment_3_areabetween")
        self.grade_assignment_3_areabetween()
        print("grade_assignment_4")
        self.grade_assignment_4()
        print("grade_assignment_5_area")
        self.grade_assignment_5_area()
        print("grade_assignment_5_fit")
